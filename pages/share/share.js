Page({
  ...
  drawQR: function (ctx, sRes) {
    ...
    var qrWidth = 100
    // 小程序码边距
    var qrMargin = that.data.screenHeight * 0.1
    // 小程序码路径
    ...
    ctx.setLineWidth(1)
    ctx.beginPath()
    ctx.arc(that.data.screenWidth  - qrMargin, that.data.screenHeight - qrWidth / 2 - qrMargin, qrWidth / 2, 0, 2 * Math.PI)
    ctx.setStrokeStyle("#ffff00")
    ctx.stroke()
    ctx.clip()
    ctx.drawImage(qrPath, that.data.screenWidth - qrWidth/2 - qrMargin, that.data.screenHeight - qrWidth - qrMargin, qrWidth, qrWidth)
    ctx.save()
